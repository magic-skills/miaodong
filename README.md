...
- **花钱要你点头**：试跑超过单次门槛（默认 ¥2）、当天累计超过上限（默认 ¥10）、估不出花费、会调用插件，或者跑到一半按实际花费算超了，md 都会先停下，报出预估金额和一个确认码；AI 要单独问你，你同意后它才能带着确认码去跑。跑的过程中每一次都按实际花费重算，超了就停。这是给 AI 定的规矩，不是锁（确认码只有 AI 看得到），所以 `md spend` 里每一笔都标着是自动放行还是经你确认的，随时可以查。
